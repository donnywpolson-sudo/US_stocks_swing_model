...
from scripts.phase8_model_selection.feature_discovery import load_feature_selection_config
...
def run_feature_selection(paths: ProjectPaths | None = None) -> dict[str, object]:
    ...
    if not discovery_path.exists():
        raise FileNotFoundError(
            ...
            "Run: python scripts/phase8_model_selection/discover_features.py --max-folds 2"
        )
...
def freeze_feature_set(paths: ProjectPaths | None = None) -> dict[str, object]:
    ...
    if missing:
        raise FileNotFoundError(
            ...
            + ", ".join(missing)
            + ". Run: python scripts/phase8_model_selection/select_features.py"
        )
