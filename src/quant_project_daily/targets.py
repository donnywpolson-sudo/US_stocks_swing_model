...
def generate_targets(
    ...
    *,
    ...
    df["exit_close_20d"] = grp["close"].shift(-horizon_days)
    df["exit_date_20d"] = grp["date"].shift(-horizon_days)
    df["fwd_ret_20d"] = (df["exit_close_20d"] / df["next_open"]) - 1.0
    ...
    df["has_split_like_gap_in_target_window_20d"] = (
    ...
    is_missing_future = df["next_open"].isna() | df["exit_close_20d"].isna()
    is_excluded = df["ticker"].isin(excluded)
    has_split_window = df["has_split_like_gap_in_target_window_20d"]

    df["label_valid_20d"] = ~(is_pre_research | is_non_model | is_missing_future | is_excluded | has_split_window)

    df["target_class_20d"] = 0
    valid_idx = df["label_valid_20d"]
    if valid_idx.any():
        classes = df.loc[valid_idx].groupby("date", group_keys=False)["fwd_ret_20d"].apply(
            lambda s: _assign_date_classes(s, top_bottom_quantile)
        )
        df.loc[classes.index, "target_class_20d"] = classes.astype("int8")

    df["target_long_top20_20d"] = df["target_class_20d"].eq(1)
    df["target_short_bottom20_20d"] = df["target_class_20d"].eq(-1)
    df["_valid_target_class"] = df["target_class_20d"].where(df["label_valid_20d"])
    df["exit_date_20d"] = pd.to_datetime(df["exit_date_20d"]).dt.date
    ...
    by_year = (
        ...
        .agg(
            ...
            label_valid_rows=("label_valid_20d", "sum"),
    ...
    class_counts = {str(int(k)): int(v) for k, v in df.loc[df["label_valid_20d"], "target_class_20d"].value_counts().sort_index().items()}
    ...
    summary = {
        ...
        "label_valid_rows": int(df["label_valid_20d"].sum()),
        "invalid_reason_counts_are_overlapping": True,
...
def run_targets(paths: ProjectPaths | None = None) -> dict[str, object]:
    ...
    reset_parquet_output_dir(p.labeled_target_h20)
    if not result.data.empty:
        result.data.to_parquet(p.labeled_target_h20 / "targets.parquet", engine="pyarrow", index=False)

    p.label_reports.mkdir(parents=True, exist_ok=True)
    (p.label_reports / "target_h20_summary.json").write_text(
        json.dumps(result.summary, indent=2, default=str),
        encoding="utf-8",
    )
    result.by_year.to_csv(p.label_reports / "target_h20_by_year.csv", index=False)
    result.by_date.to_csv(p.label_reports / "target_h20_by_date.csv", index=False)
