...
def _make_paths(tmp_path: Path) -> ProjectPaths:
    return ProjectPaths(
        ...
        oos_predictions_baseline_h20=tmp_path / "oos_preds",
            ...
            rows.append(
                {
                    ...
                    "ticker": f"TICK{i}",
                    "raw_ticker": f"TICK{i}",
                    "target_class_20d": 1 if i % 2 == 0 else 0,
                    "fwd_ret_20d": float(rng.uniform(-0.05, 0.08)),
                    "pred_score_20d": float(rng.uniform(-3.0, 3.0)),
                    "pred_rank_pct_by_date": float(rng.uniform(0, 1)),
                    "pred_long_rank_20d": i + 1,
                    "pred_short_rank_20d": i + 1,
                }
            )
...
class TestBuildMetrics:
    ...
    def test_null_prediction_rows_triggers_warning(self) -> None:
        ...
        df.loc[0, "pred_score_20d"] = np.nan
        df.loc[1, "fwd_ret_20d"] = np.nan
...
class TestRunMetrics:
    def test_run_metrics_integration(self, tmp_path: Path) -> None:
        ...
        paths.oos_predictions_baseline_h20.mkdir(parents=True, exist_ok=True)
        paths.metrics_reports.mkdir(parents=True, exist_ok=True)

        df = _make_pred_rows(60, n_dates=3, n_folds=3)
        df.to_parquet(paths.oos_predictions_baseline_h20 / "preds.parquet")

        summary = run_metrics(paths=paths)

        assert (paths.metrics_reports / "baseline_h20_metrics_summary.json").exists()

        csv_files = list(paths.metrics_reports.glob("baseline_h20_*.csv"))
